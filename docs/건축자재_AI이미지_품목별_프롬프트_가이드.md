...
## Veo(이미지→영상)용 Start Frame 별도 제작 규칙 — 신설 제안

> 근거 프로젝트: `uv-coating-shorts-v1`  
> 작업 날짜: 2026-08-27  
> 근거 수준: 단일 Scene의 v1/v2 비교 검증. 공통 규칙 승격 전 추가 재현 검토 필요.

정지 이미지의 완성도와 Veo Start Frame의 적합성은 별도로 판단한다. 정지 이미지에서 사실적으로 보이는 작고 반복적인 기계 구조물도 영상화 과정에서는 프레임 간 일관성이 무너질 수 있다.

### 확인된 실패·개선 사례

- `scene04-uv-line-v2.png`를 사용한 최초 영상에서는 판재가 수평 레일을 따르지 않고 위로 밀리는 수직 이동이 발생했다.
- 카메라 고정, 일정 높이, 수직 이동·줌 금지, 직선 수평 이동을 모션 프롬프트에 명시한 재테스트에서는 수직 이동 문제가 확실히 개선됐다.
- 그러나 판재 진입부 근처의 롤러 받침대, 브래킷, 힌지 등 복잡한 소형 구조물은 프레임 진행 중 미세하게 뒤틀렸다.
- 이 현상은 실패 패턴 DB의 `복잡한 구조 디테일 붕괴` 계열로 분류한다.
- 해당 이미지는 정지 이미지용으로는 적합하지만 Veo 영상화용 Start Frame으로는 부적합하다고 판정한다.

### Start Frame 제작 제안 규칙

1. 움직이는 오브젝트(판재 등)는 단순하고 평평한 형태로 생성한다.
2. 움직이지 않는 배경·기계는 디테일을 최소화하거나 아웃포커스로 처리한다.
3. 이동 경로 주변의 복잡한 브래킷·롤러·힌지는 화면 밖으로 빼거나 구조를 단순화한다.
4. 모션 프롬프트에는 카메라 고정 여부, 이동축, 일정 높이, 이동 속도와 금지 모션을 명시한다.
5. 정지 이미지 승인과 별도로 `Veo Start Frame 적합성`을 검수한다.
