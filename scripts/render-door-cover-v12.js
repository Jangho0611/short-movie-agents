...
async function main() {
  ...
  const overlay = Buffer.from(`
    <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="${boxLeft}" y="${boxTop}" width="${boxRight-boxLeft}" height="${boxBottom-boxTop}" rx="32" fill="${BLOCK_MINT}"/>
      <text x="92" y="445" font-family="${FONT}" font-size="60" font-weight="800" letter-spacing="-1.5" fill="${INK}">${esc('도어 발주,')}</text>
      <text x="92" y="537" font-family="${FONT}" font-size="60" font-weight="800" letter-spacing="-1.5" fill="${INK}">${esc('이것만 알면')}</text>
  ...
  const target = path.join(OUT, 'door-order-cover-v12.png');
