...
async function main() {
  ...
  const overlay = Buffer.from(`
    <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="${boxLeft}" y="${boxTop}" width="${boxRight-boxLeft}" height="${boxBottom-boxTop}" rx="36" fill="${DEEP_GREEN}" fill-opacity="0.94"/>
      ...
      <text x="92" y="629" font-family="${FONT}" font-size="60" font-weight="800" letter-spacing="-1.5" fill="${MINT}">${esc('안 틀려요')}</text>
  ...
  const target = path.join(OUT, 'door-order-cover-v11.png');
  ...
  console.log(JSON.stringify({target, charWidth, charHeight, charTop}, null, 2));
}
