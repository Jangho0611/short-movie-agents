...
# NOTE: storyboard_agent still imports vertexai at module load time, which
# requires GCP ADC credentials. It stays disabled so the pipeline can run
# with an AI Studio / Gemini API key only (no GCP auth). video_agent now
# uses genai.Client() in API-key mode, so it is safe to re-enable.
...
if story_agent and screenplay_agent and video_agent:
    root_agent = Agent(
        name="director_agent",
        model=MODEL,
